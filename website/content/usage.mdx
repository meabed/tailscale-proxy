...
| `--docker` | off | Also discover Docker containers via the local Docker API (read-only) |
...
| `--match-separators` | on | Route `/a-b/` and `/a_b/` alike; `--match-separators=false` for exact-dash |
